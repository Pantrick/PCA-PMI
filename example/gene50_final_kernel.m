...
TH=[0.500,0.10,0.1500, 0.275,0.400];% choose the threshold when the accuracy is the highest ;result_DREAM
THk=[0.20,0.35,0.02, 0.21,0.35];%threshold for kpca-pmi
ColorMap=hsv(3);

for i=1:numel(File)
    figure(i);
    cd(DATA50);
    %NullMU=importdata(sprintf('InSilicoSize50-%s-heterozygous.tsv',File{i}));
    if exist(sprintf('InSilicoSize50-%s-%s.tsv',File{i},Type),'file')
        NullMU=importdata(sprintf('InSilicoSize50-%s-%s.tsv',File{i},Type));
    else
        NullMU=importdata(sprintf('InSilicoSize50-%s-%s.txt',File{i},Type));
    end 
    cd(GOLD);
    ANSWER=importdata(sprintf('DREAM3GoldStandard_InSilicoSize50_%s.txt',File{i}));
    cd(CODE)
...
%     h4=line(x4,y4,'Color',ColorMap(8,:),'LineWidth',0.7,'LineStyle','--');hold on;
    ...
    title(sprintf('ROC of %s (50 Genes)',File{i}),'Color','black','FontSize',26,'FontWeight','bold');
    %box off;
   %%
    ...
    legend('boxoff');
    cd(RESULT);
    saveas(gcf,sprintf('%s_gene50_kernel.png',File{i}),'png');
    saveas(gcf,sprintf('%s_gene50_kernel.eps',File{i}),'psc2');
%     saveas(gcf,sprintf('%s_gene50.tif',File{i}),'tif');
%     saveas(gcf,sprintf('%s_gene50.emf',File{i}),'emf');
end
%%
% set(gca,'Xcolor','black','Ycolor','black','TickDir','out');
% xlabel('False Positive Rate','Color','black','FontSize',22,'FontWeight','bold');
% ylabel('True Positive Rate','Color','black','FontSize',22,'FontWeight','bold');