...
TH=[0.250,0.25,0.250, 0.3750,0.50];% choose the threshold when the accuracy is the highest ;result_DREAM
THk=[0.11,0.13,0.40, 0.11,0.30];%threshold for kpca-pmi
ColorMap=hsv(3);
...
%     h4=line(x4,y4,'Color',ColorMap(8,:),'LineWidth',0.7,'LineStyle','--');hold on;
   ...
   %%
    ...
    legend('boxoff');
    cd(RESULT);
    saveas(gcf,sprintf('%s_gene100_kernel.png',File{i}),'png');
    saveas(gcf,sprintf('%s_gene100_kernel.eps',File{i}),'psc2');
%     saveas(gcf,sprintf('%s_gene100.tif',File{i}),'tif');
%     saveas(gcf,sprintf('%s_gene100.emf',File{i}),'emf');
end
%%
